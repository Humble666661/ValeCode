...
from valecode.hooks import HookContext, HookEngine, load_hooks
from valecode.conversation import ConversationManager, Message
from valecode.mcp import ConnectResult, MCPManager
from valecode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from valecode.agents.notification import inject_task_notifications
...
from valecode.worktree.cleanup import start_stale_cleanup_task
...
from valecode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".valecode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".valecode" / "history"
...
class ValeCodeApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_VALECODE_THEME)
        self.theme = "valecode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        from valecode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _show_plan_approval(self) -> None:
        from valecode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from valecode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from valecode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from valecode.permission_dialog import InlinePermissionWidget
